...
def on_channel_open(channel):
    ...
    print('Redis-Parameters:\nHost: {}\nPort: {}\nPassword:{}'.format(
        REDIS_HOST, str(REDIS_PORT), REDIS_PASSWORD))
...
def handle_message(body, redis_connection, channel):
    ...
    stored_redis_password = redis_connection.get(md5)
    if stored_redis_password is not None:
        stored_redis_password = stored_redis_password.decode('UTF-8')
        print("Password for '{}' is '{}'".format(
            md5,
            stored_redis_password
        ))

        message['password'] = stored_redis_password
        routing_key = 'found'
    else:
        print(
